...
$arr_ac_p = create_arr_for_var($arrmain, $arrAssoc, 'ac_p');
$arr_in_f = create_arr_for_var($arrmain, $arrAssoc, 'in_f');
$arr_in_lp = create_arr_for_var($arrmain, $arrAssoc, 'in_lp');
$get_num_val_f = $arrAssoc['in_f'];//получаем номер выборки для массива по id
$get_num_val_p = $arrAssoc['ac_p'];
$get_num_val_pl = $arrAssoc['in_lp'];
 ...
 <script>
  ...
  var frequency = <?php echo json_encode($arr_in_f);?>;
  var power = <?php echo json_encode($arr_ac_p);?>;
  var power_limit = <?php echo json_encode($arr_in_lp);?>;
  var value_num_f = <?php echo json_encode($get_num_val_f) ?>;
  var value_num_p = <?php echo json_encode($get_num_val_p) ?>;
  var value_num_pl = <?php echo json_encode($get_num_val_pl) ?>;
            ...
            load: function () {
                ...
                setInterval(function () {
                    function get_data(log, value_num, series){
                      ...
                      get_data('./logs/inv_log', value_num_pl, powl);
